...
`platform/api`'s Public C API (Work Package 012: Engineering Object Enumeration, Repository Statistics) adds no new surface to `FoundationRuntime` itself — its enumeration and statistics functions are built entirely from the existing `object_store()`/`relationship_store()`/`current_metadata()`/`current_package_set()` accessors already described above, translated into fixed-layout C structures at the API boundary. `FoundationRuntime`'s own interface is unchanged by that work package.
