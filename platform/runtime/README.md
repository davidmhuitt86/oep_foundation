...
`platform/api`'s Public C API (Work Package 012: Engineering Object Enumeration, Repository Statistics; Work Package 013: Engineering Relationship Enumeration, Repository Search) adds no new surface to `FoundationRuntime` itself — every function it exposes is built entirely from the existing `object_store()`/`relationship_store()`/`search_engine()`/`current_metadata()`/`current_package_set()` accessors already described above, translated into fixed-layout C structures at the API boundary. `FoundationRuntime`'s own interface is unchanged by either work package; in particular, Work Package 013's search functions read the same `SearchEngine` instance `search_engine()` already returns — the one built by `build_index` during `open_repository` — rather than constructing or indexing anything new.
